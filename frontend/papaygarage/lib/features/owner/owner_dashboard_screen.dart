              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // ── Year selector ──────────────────────────────
                    const Text('Year', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: _muted)),
                    ...
                    // ── Month grid ────────────────────────────────
                    const Text('Month', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: _muted)),
                    const SizedBox(height: 8),
                    GridView.builder(
                      shrinkWrap: true,
                        ...
                        return GestureDetector(
                          ...
                          child: AnimatedContainer(
                            ...
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(color: selected ? _gold : _border, width: 1.5),
                            ),
    ...
    return Row(
      children: [
        ...
        Container(
          ...
          child: const Icon(Icons.bar_chart_rounded, color: _gold, size: 28),
        ),
          ...
          child: Column(
            ...
            children: [
              const Text('Owner Dashboard',
                  style: TextStyle(fontSize: 28, fontWeight: FontWeight.w800, color: _ink, letterSpacing: -0.5)),
              const SizedBox(height: 4),
              Text('Financial summary for $monthName',
    ...
    return Consumer<OwnerState>(
      builder: (context, state, _) {
        ...
        if (state.summary == null) {
          return const Center(child: Text('No data yet.', style: TextStyle(color: _muted)));
        }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: _gold.withOpacity(0.25), blurRadius: 24, offset: const Offset(0, 8))],
      ),
      child: Row(
        children: [
          Expanded(
            child: _NetItem(label: 'Total Profit', value: summary.totalProfit, light: true),
          ),
          Container(width: 1, height: 48, color: Colors.white.withOpacity(0.3)),
          Expanded(
            child: _NetItem(label: 'Total Expense', value: summary.totalExpense, light: true),
          ),
          Container(width: 1, height: 48, color: Colors.white.withOpacity(0.3)),
          Expanded(
            child: Column(
              children: [
                Text('Net', style: TextStyle(color: Colors.white.withOpacity(0.8), fontSize: 12, fontWeight: FontWeight.w600)),
...
class _NetItem extends StatelessWidget {
  ...
  final double value;
  final bool light;
  const _NetItem({required this.label, required this.value, this.light = false});

  @override
  Widget build(BuildContext context) {
    final color = light ? Colors.white : _ink;
    final mColor = light ? Colors.white.withOpacity(0.7) : _muted;
    return Column(
      children: [
        Text(label, style: TextStyle(color: mColor, fontSize: 12, fontWeight: FontWeight.w600)),
        const SizedBox(height: 6),
        Text('QAR ${_fmt.format(value)}',
            style: TextStyle(color: color, fontSize: 18, fontWeight: FontWeight.w800, letterSpacing: -0.3)),
      ],
    );
  }
}
    ...
    return Column(
      children: [
        _SectionCard(
          ...
          child: Column(
            children: [
              _ProfitBreakdownRow(breakdown: summary.insuranceProfit),
            ],
          ),
        ),
        const SizedBox(height: 16),
        ...
        _SectionCard(
          ...
          child: _AmountRow(label: 'Total', amount: summary.partsProfit, highlight: true),
        ),
      ],
    );
  }
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        Row(children: [
          Expanded(child: _BadgeAmount(label: 'Paid', amount: breakdown.paid, color: const Color(0xFF2DAA62))),
          const SizedBox(width: 10),
          Expanded(child: _BadgeAmount(label: 'Unpaid', amount: breakdown.unpaid, color: const Color(0xFFE04340))),
        ]),
      ],
    );
  }
            ...
            children: [
              if (sal.employees.isEmpty)
                const Padding(
                  ...
                  child: Text('No salaries recorded', style: TextStyle(color: _muted, fontSize: 13)),
                )
              else
  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: BoxDecoration(
        ...
        boxShadow: const [BoxShadow(color: Color(0x05211D16), blurRadius: 24, offset: Offset(0, 8))],
      ),
      child: Column(
        ...
        children: [
          Padding(
            ...
            child: Row(
              children: [
                Container(
                  width: 36, height: 36,
                  decoration: BoxDecoration(color: _goldSoft, borderRadius: BorderRadius.circular(9)),
          ...
          Padding(padding: const EdgeInsets.symmetric(horizontal: 32, vertical: 24), child: child),
        ],
      ),
    );
  }
...
class _AmountRow extends StatelessWidget {
  ...
  const _AmountRow({required this.label, required this.amount, this.highlight = false});
...
class _BadgeAmount extends StatelessWidget {
  ...
  const _BadgeAmount({required this.label, required this.amount, required this.color});
