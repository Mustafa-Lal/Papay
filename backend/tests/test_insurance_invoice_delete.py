...
from app.services.insurance_customer import create_insurance_customer
from app.services.insurance_invoice import create_insurance_invoice
...
class InsuranceInvoiceDeleteTests(unittest.TestCase):
        ...
        with Session(engine) as db:
            ...
            saved_item1 = db.scalar(
                select(InsuranceItem).where(
                    InsuranceItem.id == item1_id
                )
            )

            saved_item2 = db.scalar(
                select(InsuranceItem).where(
                    InsuranceItem.id == item2_id
                )
            )
            ...
            saved_customer = db.scalar(
                select(InsuranceCustomer).where(
                    InsuranceCustomer.id == customer.id
                )
            )
    ...
    def test_other_invoice_and_items_remain(self):
        with Session(engine) as db:
            ...
            invoice2 = create_insurance_invoice(
                db=db,
                customer_id=customer.id,
            ...
            remaining_invoice = db.scalar(
                select(InsuranceInvoice).where(
                    InsuranceInvoice.id == invoice2_id
                )
            )

            remaining_item = db.scalar(
                select(InsuranceItem).where(
                    InsuranceItem.id == item3_id
                )
            )
