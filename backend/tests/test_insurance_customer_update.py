...
class InsuranceCustomerUpdateTests(unittest.TestCase):
...
def test_update_can_be_rolled_back(self):
    with Session(engine) as db:
        ...
        # Create and commit the original customer first.
        customer = self.create_test_customer(db)
        ...
        # Start a new transaction and update the customer.
        update_customer(
            db=db,
            customer_id=customer_id,
        ...
        # Clean up the committed test customer.
        db.delete(saved_customer)
        db.commit()
